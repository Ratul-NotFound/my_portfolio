...
import {
  ...
  Compass, Briefcase, Code, Phone, Users, Heart, CheckCircle
} from 'lucide-react';
...
const heroBadgeReveal = {
  ...
  show: { opacity: 1, x: 0, y: 0, transition: { duration: 0.6, delay: 0.05, ease: [0.22, 1, 0.36, 1] } }
};
...
const TechGrid = ({ skills, isHacker, isLight, isCreative }) => {
  const ref = useRef(null);
  const { scrollYProgress } = useScroll({
    target: ref,
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center gap-4">
            <div className="relative w-16 h-16 flex items-center justify-center">
              ...
              <div className={`relative w-full h-full p-2 rounded-xl border transition-all group-hover/card:scale-110 ${isHacker ? 'bg-[#000a02] border-[#00ff41]/15 group-hover/card:border-[#00ff41]/30' : isCreative ? 'bg-[#0c0c0c] border-white/10 group-hover/card:border-[#0088ff]/30 shadow-sm' : isLight ? 'bg-white border-slate-200 group-hover/card:border-indigo-300 shadow-sm' : 'bg-slate-800/50 border-slate-700 group-hover/card:border-slate-600'}`}>
                <img
                  src={skill.icon}
                  alt={skill.name}
          ...
          <div className={`absolute bottom-0 left-0 right-0 h-[2px] transform scale-x-0 group-hover/card:scale-x-100 transition-transform duration-500 ${isHacker ? 'bg-[#00ff41]' : isCreative ? 'bg-[#0088ff]' : 'bg-gradient-to-r from-cyan-500 via-purple-500 to-pink-500'}`}></div>
        </ScrollSkillCard>
      ))}
    </div>
  );
};
const ShufflingProjectWrapper = ({ children, index, total, progress, activeProject }) => {
...
function SectionBadge({ icon: Icon, label, isHacker, isLight, isCreative }) {
  const ref = useRef(null);
...
function SectionTitleLine({ isHacker, isLight, isCreative }) {
  const ref = useRef(null);
  ...
  return (
    <motion.div
      ...
      className={`h-[2px] rounded-full mx-auto mb-4 ${
        isHacker ? 'bg-[#00ff41]/40' : isCreative ? 'bg-[#0088ff]' : isLight ? 'bg-indigo-300' : 'bg-gradient-to-r from-cyan-500 via-purple-500 to-pink-500'
      }`}
...
function HoloProfileCard({ isLight, isHacker, isCreative }) {
  ...
  const themeColors = {
    spotlight: isHacker ? "rgba(0, 255, 65, 0.12)" : isCreative ? "rgba(0, 136, 255, 0.15)" : isLight ? "rgba(79, 70, 229, 0.12)" : "rgba(34, 211, 238, 0.12)",
    accent: isHacker ? "#00ff41" : isCreative ? "#0088ff" : isLight ? "#4f46e5" : "#22d3ee",
    scanner: isHacker ? "rgba(0, 255, 65, 0.5)" : isCreative ? "rgba(0, 136, 255, 0.5)" : isLight ? "rgba(79, 70, 229, 0.5)" : "rgba(34, 211, 238, 0.6)"
  };
  ...
  return (
    <motion.div
      ...
      className={`group relative w-full max-w-lg mx-auto backdrop-blur-2xl border rounded-3xl p-6 md:p-8 space-y-6 transition-all duration-500 cursor-default select-none overflow-hidden ${
        isHacker
          ...
          : isCreative
            ? 'bg-[#0a0a0a]/90 border-white/10 hover:border-[#0088ff]/40 shadow-xl hover:shadow-[#0088ff]/10'
            : isLight
    ...
    >
      ...
      <div className="absolute top-3 left-4 font-mono text-[8px] opacity-35 tracking-wider">
        {isCreative ? "STUDIO.HUD // DHAKA.BD" : "SYS.HUD // DHAKA.BD [N 23° 42' E 90° 22']"}
      </div>
      <div className="absolute top-3 right-4 font-mono text-[8px] opacity-35 tracking-wider">
        {isCreative ? "EST.2026 // CREATIVE_CON" : "VER.2.08 // SECURE_CON"}
      </div>

      {/* Interactive Tabs Header Control Panel */}
      <div className="relative z-10 flex items-center justify-between border-b pb-4 mt-2" style={{ borderColor: isHacker ? 'rgba(0,255,65,0.1)' : isCreative ? 'rgba(255,255,255,0.08)' : isLight ? '#f1f5f9' : 'rgba(255,255,255,0.05)' }}>
            ...
            return (
              ...
              >
                <span className="hidden sm:inline">
                  [{tab === 'avatar' ? (isCreative ? 'STUDIO_AVATAR' : 'HOLO_AVATAR') : tab === 'metrics' ? 'SYS_STATS' : 'CODE_KERNEL'}]
                </span>
              ...
              {metrics.map((metric, i) => (
                <div key={i} className="space-y-1">
                  <div className="flex justify-between items-center text-xs md:text-sm font-mono font-bold">
                    <span className={isHacker ? 'text-[#00cc32]' : isCreative ? 'text-white' : isLight ? 'text-slate-700' : 'text-slate-300'}>{metric.name}</span>
                    <span style={{ color: themeColors.accent }}>{metric.val}%</span>
                  </div>
                  {/* Gauge Tracker */}
                  <div className={`h-2 rounded-full overflow-hidden w-full relative ${isHacker ? 'bg-[#00ff41]/5' : isCreative ? 'bg-white/10' : isLight ? 'bg-slate-100' : 'bg-slate-900'}`}>
                    <motion.div 
            ...
            <motion.div
              ...
              className={`p-4 rounded-xl border font-mono text-[10px] md:text-xs text-left overflow-auto lg:max-h-[355px] md:max-h-[295px] max-h-[235px] w-full relative z-10 transition-colors duration-500 ${
                isHacker 
                  ...
                  : isCreative
                    ? 'bg-[#0a0a0a] border-white/10 text-slate-300'
                    : isLight 
                      ? 'bg-slate-50 border-slate-200 text-slate-650' 
                      : 'bg-[#0b0f19] border-slate-900 text-cyan-400'
              }`}
            >
              <div className="opacity-45 border-b pb-2 mb-2 select-none flex justify-between text-[9px]" style={{ borderColor: isCreative ? 'rgba(255,255,255,0.08)' : '' }}>
...
function CompetencyConsole({ isLight, isHacker, isCreative }) {
  ...
  const competencies = useMemo(() => [
    {
      ...
      color: isHacker ? "#00ff41" : isCreative ? "#0088ff" : isLight ? "#4f46e5" : "#22d3ee",
      glow: isHacker ? "rgba(0, 255, 65, 0.15)" : isCreative ? "rgba(0, 136, 255, 0.15)" : isLight ? "rgba(79, 70, 229, 0.15)" : "rgba(34, 211, 238, 0.15)"
    },
    {
      icon: Brain,
      index: "02",
      ...
      color: isHacker ? "#00ff66" : isCreative ? "#00d8b4" : isLight ? "#9333ea" : "#a855f7",
      glow: isHacker ? "rgba(0, 255, 102, 0.15)" : isCreative ? "rgba(0, 216, 180, 0.15)" : isLight ? "rgba(147, 51, 234, 0.15)" : "rgba(168, 85, 247, 0.15)"
    },
    {
      icon: Zap,
      index: "03",
      ...
      color: isHacker ? "#00c8ff" : isCreative ? "#8b5cf6" : isLight ? "#ec4899" : "#f43f5e",
      glow: isHacker ? "rgba(0, 200, 255, 0.15)" : isCreative ? "rgba(139, 92, 246, 0.15)" : isLight ? "rgba(236, 72, 153, 0.15)" : "rgba(244, 63, 94, 0.15)"
    },
    {
      icon: BookOpen,
      index: "04",
      ...
      color: isHacker ? "#ffd700" : isCreative ? "#f59e0b" : isLight ? "#f59e0b" : "#eab308",
      glow: isHacker ? "rgba(255, 215, 0, 0.15)" : isCreative ? "rgba(245, 158, 11, 0.15)" : isLight ? "rgba(245, 158, 11, 0.15)" : "rgba(234, 179, 8, 0.15)"
    }
  ], [isLight, isHacker, isCreative]);
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-8 items-stretch pt-6 pb-12 select-none">
      ...
      <div className="lg:col-span-5 flex flex-col justify-center gap-4">
        <div className="mb-2">
          <span className={`text-[10px] font-mono tracking-[0.25em] font-black uppercase ${isHacker ? 'text-[#00ff41]/50' : isCreative ? 'text-[#8c6e58]/70' : 'text-slate-400'}`}>
              ...
              className={`group flex items-center justify-between p-3.5 md:p-5 rounded-2xl border text-left transition-all duration-550 w-full relative overflow-hidden backdrop-blur-md outline-none ${
                isSelected
                  ? isHacker 
                    ...
                    : isCreative
                      ? 'bg-[#0a0a0a]/90 border-[#0088ff] shadow-xl shadow-[#0088ff]/10'
                      : isLight 
                  ...
                  : isHacker
                    ...
                    : isCreative
                      ? 'bg-[#0a0a0a]/40 border-white/5 hover:border-[#0088ff]/30 hover:bg-[#0a0a0a]/80 shadow-sm'
                      : isLight
        ...
        <div 
          className={`flex-1 rounded-3xl border backdrop-blur-xl p-6 md:p-8 flex flex-col justify-between relative overflow-hidden transition-all duration-700 ${
            isHacker 
              ...
              : isCreative
                ? 'bg-[#0a0a0a]/90 border-white/10 shadow-xl hover:shadow-[#0088ff]/10'
                : isLight 
        ...
        >
          ...
          <div className={`mt-6 p-4 rounded-xl border font-mono text-[10px] md:text-xs relative z-10 transition-colors duration-500 overflow-hidden ${
            isHacker 
              ? 'bg-[#000a02] border-[#00ff41]/15 text-[#00ff41]' 
              : isCreative
                ? 'bg-[#0a0a0a] border-white/10 text-[#0088ff]'
                : isLight 
...
// ── StatCard with count-up ──
function StatCard({ stat, isLight, isHacker, isCreative }) {
  const cardRef = useRef(null);
    ...
    <motion.div
      ...
      className={`group relative p-6 md:p-8 backdrop-blur-xl border rounded-2xl cursor-default overflow-hidden transition-all duration-500 ${
        isHacker
          ...
          : isCreative
            ? 'bg-[#0a0a0a]/90 border-white/10 hover:border-[#0088ff] shadow-lg hover:shadow-[#0088ff]/10'
            : isLight
              ? 'bg-white/40 border-slate-200/80 shadow-lg shadow-indigo-500/5 hover:border-indigo-400'
              : 'bg-[#0b0f19]/40 border-slate-900 shadow-2xl hover:border-cyan-500/30'
      }`}
    >
        ...
        style={{
          backgroundImage: isHacker
            ...
            : `radial-gradient(circle 140px at var(--spotlight-x, 50%) var(--spotlight-y, 50%), ${
                isCreative ? 'rgba(0, 136, 255, 0.15)' : isLight ? 'rgba(99, 102, 241, 0.15)' : 'rgba(34, 211, 238, 0.12)'
              }, transparent 80%)`
        }}
      />
      ...
      <div className="relative z-10 flex flex-col justify-between h-full" style={{ transform: 'translateZ(30px)' }}>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className={`p-3 rounded-xl transition-all duration-300 group-hover:scale-110 group-hover:rotate-6 ${
            isHacker ? 'bg-[#00ff41]/5 text-[#00ff41]' : isCreative ? 'bg-[#0088ff]/5 text-[#0088ff]' : isLight ? 'bg-indigo-50 text-indigo-650' : 'bg-cyan-500/10 text-cyan-400'
          }`}>
            <stat.icon className="w-6 h-6 animate-pulse" />
          </div>
          
          {/* Active status pulse */}
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-dashed text-[9px] font-mono font-bold tracking-wider uppercase select-none pointer-events-none leading-none scale-90 origin-right opacity-60 group-hover:opacity-100 transition-opacity duration-300" style={{ borderColor: isCreative ? 'rgba(0,136,255,0.2)' : '' }}>
            <span className={`w-1.5 h-1.5 rounded-full animate-ping ${isHacker ? 'bg-[#00ff41]' : isCreative ? 'bg-[#00d8b4]' : isLight ? 'bg-indigo-600' : 'bg-cyan-400'}`} />
            <span className={isHacker ? 'text-[#00ff41]' : isCreative ? 'text-[#00d8b4]' : isLight ? 'text-indigo-650' : 'text-cyan-400'}>Live</span>
          </div>
        </div>

        {/* Numbers & Titles */}
        <div className="space-y-1.5">
          <div className={`stat-number text-4xl md:text-5xl font-black tracking-tight leading-none select-none ${
            isCreative ? 'text-[#0088ff]' : 'bg-gradient-to-r bg-clip-text text-transparent'
          } ${
            isHacker
          ...
          <div className={`text-[10px] font-black font-mono tracking-[0.2em] uppercase select-none ${
            isHacker ? 'text-[#00cc32]/60' : isCreative ? 'text-[#0088ff] cursive-accent' : isLight ? 'text-slate-550 font-extrabold' : 'text-slate-300'
          }`}>
            {stat.label}
          </div>
          
          <div className={`text-xs select-none ${
            isHacker ? 'text-[#00ff41]/30' : isCreative ? 'text-slate-450' : isLight ? 'text-slate-400' : 'text-slate-500'
          }`}>
            {stat.desc}
          </div>
        </div>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r ${
        isHacker 
          ...
          : isCreative
            ? 'from-transparent via-[#c44b31]/50 to-transparent'
            : isLight 
...
const InteractiveText = ({ text, className, isHacker, isLight, isCreative }) => {
  return (
          ...
          {word.split('').map((char, ci) => (
            <motion.span
              ...
              whileHover={{
                ...
                color: isHacker ? '#00ff41' : isCreative ? '#0088ff' : isLight ? '#4f46e5' : '#22d3ee',
                textShadow: isHacker ? '0 0 12px rgba(0, 255, 65, 0.9)' : isCreative ? '0 0 12px rgba(0, 136, 255, 0.9)' : isLight ? '0 0 10px rgba(79, 70, 229, 0.5)' : '0 0 12px rgba(34, 211, 238, 0.9)'
              }}
...
const HandshakeConnector = ({ isHacker, isLight, isCreative }) => {
    ...
    <motion.div 
      className={`relative w-full max-w-md mx-auto h-[240px] rounded-2xl border overflow-hidden flex flex-col items-center justify-center p-4 transition-all duration-500 cursor-pointer group/handshake ${
        isHacker
          ...
          : isCreative
            ? 'bg-[#0a0a0a]/90 border-white/10 hover:border-[#0088ff] shadow-xl hover:shadow-[#0088ff]/10'
            : isLight
      ...
      <div 
        ...
        style={{
          backgroundImage: isHacker
            ...
            : `radial-gradient(circle 130px at 50% 50%, ${isCreative ? 'rgba(0, 136, 255, 0.15)' : isLight ? 'rgba(99, 102, 241, 0.15)' : 'rgba(34, 211, 238, 0.12)'}, transparent 80%)`
        }}
      />
            ...
            <motion.div
              ...
              className={`absolute w-10 h-10 rounded-full border pointer-events-none z-10 ${
                isHacker 
                  ...
                  : isCreative
                    ? 'border-[#c44b31] bg-[#c44b31]/20 shadow-[0_0_15px_rgba(196,75,49,0.5)]'
                    : isLight 
          ...
          <div className="absolute text-center select-none pointer-events-none">
            <motion.div
              ...
              className={`w-5 h-5 rounded-full border border-dashed mx-auto mb-1 flex items-center justify-center ${
                isHacker ? 'border-[#00ff41]/40' : isCreative ? 'border-[#0088ff]/40' : isLight ? 'border-indigo-300' : 'border-cyan-500/40'
              }`}
            >
              <div className={`w-2 h-2 rounded-full ${isHacker ? 'bg-[#00ff41]/50' : isCreative ? 'bg-[#0088ff]' : isLight ? 'bg-indigo-550' : 'bg-cyan-400'}`} />
            </motion.div>
            <p className={`text-[9px] font-bold tracking-widest uppercase ${
              isHacker ? 'text-[#00cc32]/60' : isCreative ? 'text-[#0088ff] cursive-accent' : 'text-slate-400'
            }`}>
              Tap to Connect
            </p>
          </div>
        ...
        {isConnected && (
          ...
          >
            <Sparkles className={`w-6 h-6 animate-pulse ${
              isHacker ? 'text-[#00ff41]' : isCreative ? 'text-[#0088ff]' : isLight ? 'text-amber-500' : 'text-cyan-300'
            }`} />
          </motion.div>
        )}
      </div>
                ...
                <a
                  ...
                  className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs transition-all hover:scale-105 ${
                    isHacker
                      ...
                      : isCreative
                        ? 'bg-[#0088ff] hover:bg-[#0077ee] text-white shadow-md'
                        : isLight
                  ...
                  className={`px-5 py-2.5 rounded-xl font-bold text-xs border transition-all ${
                    copied
                      ...
                      : isHacker
                        ...
                        : isCreative
                          ? 'bg-[#0a0a0a] border-white/10 text-slate-350 hover:border-[#0088ff] hover:text-[#0088ff]'
                          : isLight
...
const HeroTypewriter = ({ isHacker, isLight, isCreative }) => {
  ...
  const phrases = useMemo(() => {
    if (isHacker) {
      return [
        "Initializing neural subnetworks...",
        "Executing low-level secure pipelines...",
        "Compiling deep-learning models...",
        "Establishing decrypted connections..."
      ];
    }
    if (isCreative) {
      return [
        "Crafting premium minimalist interfaces...",
        "Painting terracotta and sage-toned canvases...",
        "Sculpting responsive human-centric designs...",
        "Curating bespoke editorial typographies..."
      ];
    }
    return [
...
const Portfolio = () => {
  ...
  const info = {
    ...
    address: 'YKSG-1, Daffodil Smart City, Savar, Dhaka',
    phone: '+8801784090278',
    email: 'm.h.ratul18@gmail.com',
    github: 'github.com/ratul-notfound',
    linkedin: 'linkedin.com/in/mahmud-hasan-ratul',
    facebook: 'facebook.com/mahmudhasan.ratul',
    summary: 'A dedicated Computer Science and Engineering student with a responsible and organized approach to work. I strive to apply my technical knowledge and problem-solving skills to contribute effectively in a professional environment.'
  };

  const skills = [
    { category: 'Programming', items: ['C Programming', 'App Development (Java)', 'Web Browsing'] },
    { category: 'Office Suite', items: ['MS Word', 'Google Docs', 'MS Excel', 'Google Sheets'] },
    { category: 'Presentation', items: ['MS PowerPoint', 'Google Slides', 'Canva'] },
    { category: 'Creative & Media', items: ['Video Editing'] }
  ];

  const softSkills = [
    'Communication Skill', 'Leadership Skill', 'Team Management Skill',
    'Problem Solving Skill', 'Time Management Skill', 'Public Speaking Skill'
  ];

  const languages = [
    { name: 'Bangla', level: 'Native (Fluent)' },
  ...
  const education = [
    ...
    },
    {
      ...
      institution: 'Suti V.M. Pilot Model Govt. High School',
  ...
  const experience = [
    {
      role: 'Vice President',
      org: 'DIU Computer & Programming Club (DIUCPC)',
      duration: 'Jan 2023 - Present',
      bullets: [
        'Directing club activities, organizing workshops, and mentoring aspiring developers.',
        'Served as Former Assistant General Secretary, managing active community pipelines.'
      ]
    },
    {
      ...
      duration: 'Dec 2024',
      bullets: [
        'Coordinated venue setups, volunteer pipelines, and crowd operations for 100+ competing universities.'
      ]
    },
    {
      role: 'Marketing Executive',
      org: 'The Premium Homes Ltd.',
      duration: 'Nov 2024 - 2025',
      bullets: [
        'Designed real estate branding and coordinated digital promotional campaigns.'
      ]
    },
    {
      role: 'Head of Branding and Production',
      org: 'Hult Prize at Daffodil International University',
      duration: '2024',
      bullets: [
        'Led production design and dynamic video advertisements for the campus competition.'
      ]
    }
  ];

  const volunteer = [
    { role: 'Volunteer', org: 'Bangladesh Olympiad in Informatics (BDOI)', duration: '2024 - 2025' },
    { role: 'Campus Ambassador', org: 'Jobsnavi', duration: '2024' },
    { role: 'Conference Volunteer', org: 'BIM 2023 (Int. Conf. on Big Data, IoT & ML)', duration: '2023' },
    { role: 'General Member', org: 'DIU Blood Donors Club (DIU BDC)', duration: '2023 - Present' }
  ];

  const hobbies = [
    'Singing', 'Playing Guitar', 'Cycling', 'Reading Books', 'Crafting',
    'DIY Projects', 'Photography', 'Videography', 'Playing Cricket'
  ];

  const references = [
    {
      name: 'Md. Mainul Hasan Dulon',
      title: 'CEO, The Premium Homes Ltd.',
      phone: '+880 1933-657804'
    },
    {
      name: 'Mr. Saiful Islam',
      title: 'Assistant Professor, Department of CSE, Daffodil International University',
  ...
  const pointerPercentage = useTransform(snappyProjectsProgress, [0, 1], ["0%", "100%"]);
  ...
  useMotionValueEvent(snappyProjectsProgress, "change", (latest) => {
    ...
    const newActiveId = projects[activeIndex].id;
    if (activeProjectRef.current !== newActiveId) {
      activeProjectRef.current = newActiveId;
  ...
  const projects = useMemo(() => ([
    ...
    },
    {
      ...
      description: "A fast-paced satirical arcade game capturing the chaos of Dhaka elections. Play as Mirza Abbas or Nasir Uddin Patwary and battle with eggs, tomatoes, sandals, and bananas to defend your high-stakes Izzat Meter!",
      ...
      metrics: ["50K+ Players", "Izzat Meter", "Satirical Arcade", "Comedic UI"],
      image: "/election.png",
  ...
  return (
    <div className={`min-h-screen ${isHacker ? 'bg-[#000600] text-[#00ff41]' : isCreative ? 'bg-[#030303] text-white' : isLight ? 'bg-[#f8fafc] text-slate-900' : 'bg-slate-950 text-white'} relative`}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        {isCreative && (
          <>
            <div className="absolute top-1/3 right-1/4 w-72 h-72 bg-[#0088ff]/4 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s', animationDuration: '5s' }}></div>
            <div className="absolute bottom-1/4 left-1/3 w-80 h-80 bg-[#00d8b4]/4 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1.5s', animationDuration: '6s' }}></div>
          </>
        )}
        {isHacker && (
          <>
        ...
        <div
          ref={spotlightRef}
          className={`absolute w-[500px] h-[500px] bg-gradient-radial rounded-full blur-3xl pointer-events-none transition-all duration-700 ${isHacker ? 'from-[#00ff41]/8 to-transparent' : isCreative ? 'from-[#0088ff]/8 to-transparent' : isLight ? 'from-cyan-300/15 via-blue-200/8 to-transparent' : 'from-cyan-500/10 to-transparent'}`}
          style={{
            left: '50%',
            top: '50%',
      ...
      <main className="relative z-10">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`fixed left-5 top-1/2 -translate-y-1/2 z-40 hidden xl:flex flex-col items-center gap-3 section-label ${isHacker ? 'text-[#00ff41]/35' : isCreative ? 'text-[#0088ff]/40' : isLight ? 'text-slate-300' : 'text-slate-600'}`}
          >
            <div className={`w-px h-14 ${isHacker ? 'bg-[#00ff41]/20' : isCreative ? 'bg-[#0088ff]/20' : isLight ? 'bg-slate-300' : 'bg-slate-700'}`} />
          ...
          <div className="max-w-7xl mx-auto w-full">
            <div className="grid lg:grid-cols-2 gap-6 lg:gap-8 items-center">
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <HeroTypewriter isHacker={isHacker} isLight={isLight} isCreative={isCreative} />
                  </motion.div>
              ...
              <motion.div
                ...
                viewport={{ once: true, margin: '-80px' }}
              >
                <HoloProfileCard isLight={isLight} isHacker={isHacker} isCreative={isCreative} />
              </motion.div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mt-20">
              {stats.map((stat, i) => (
                ...
                >
                  <StatCard stat={stat} isLight={isLight} isHacker={isHacker} isCreative={isCreative} />
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section id="about" ref={aboutRef} className="py-10 md:py-16 px-2 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative z-10">
            <div className="text-center mb-10">
              <SectionBadge icon={Sparkles} label="About Me" isHacker={isHacker} isLight={isLight} isCreative={isCreative} />
            ...
            <CompetencyConsole isLight={isLight} isHacker={isHacker} isCreative={isCreative} />
          ...
          <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden py-4 md:py-8 z-10">
            ...
            >
              {projects.map((project, index) => {
                ...
                const isVisible = index >= activeIndex - 1 && index <= activeIndex + 3;
                return (
                    ...
                    >
                      ...
                      <div className="relative w-full md:w-[42%] h-[160px] md:h-full bg-slate-800 overflow-hidden shrink-0">
                        ...
                        <div className="absolute top-4 right-4 w-10 h-10 bg-black/45 backdrop-blur-md border border-white/10 rounded-xl p-2 group-hover:scale-110 transition-all duration-500 shadow-xl flex items-center justify-center z-10">
                          <div className={`absolute inset-0 border border-dashed rounded-lg opacity-25 pointer-events-none ${isActive ? 'animate-spin' : ''}`} style={{ borderColor: isHacker ? '#00ff41' : isLight ? '#4f46e5' : '#22d3ee', animationDuration: '8s' }} />
                          <img
                            src={project.logo}
        ...
        <section id="skills" className="py-10 md:py-16 px-2 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative z-10">
            <div className="text-center mb-8 md:mb-10">
              <SectionBadge icon={Zap} label="Tech Stack" isHacker={isHacker} isLight={isLight} isCreative={isCreative} />
              <SectionTitleLine isHacker={isHacker} isLight={isLight} isCreative={isCreative} />
              <ParallaxHeading>
              ...
              {Object.entries(techStack).map(([category, skills]) => (
                (activeTech === 'all' || activeTech === category) && (
                  ...
                  >
                    ...
                    <TechGrid skills={skills} isHacker={isHacker} isLight={isLight} isCreative={isCreative} />
                  </div>
                )
              ))}
            </div>
            ...
            <div className="text-center mt-16">
              <div className={`inline-flex items-center gap-2 px-6 py-3 backdrop-blur-xl border rounded-full text-sm ${isHacker ? 'bg-[#000a02]/80 border-[#00ff41]/15 text-[#00cc32]/60' : isCreative ? 'bg-[#0a0a0a]/80 border-white/10 text-slate-400' : isLight ? 'bg-white/70 border-slate-200 text-slate-500' : 'bg-slate-900/50 border-slate-800 text-slate-400'}`}>
                <Sparkles className={`w-4 h-4 ${isHacker ? 'text-[#00ff41]' : isCreative ? 'text-[#0088ff]' : isLight ? 'text-indigo-400' : 'text-cyan-400'}`} />
              ...
              <div className="relative z-10 text-center space-y-6">
                <div className="space-y-3">
                  <h2 className="text-3xl md:text-4xl font-black tracking-tight leading-tight">
                    <InteractiveText text="Let's Build Something" isHacker={isHacker} isLight={isLight} isCreative={isCreative} />
                    <br className="my-1" />
