...
export default function Footer() {
  ...
  const socialLinks = [
    { icon: Github, href: 'https://github.com/ratul-notfound', label: 'GitHub', color: isHacker ? 'hover:text-[#00ff41]' : isLight ? 'hover:text-slate-900' : 'hover:text-white' },
    { icon: Linkedin, href: 'https://linkedin.com/in/mahmud-hasan-ratul', label: 'LinkedIn', color: isHacker ? 'hover:text-[#00ff41]' : isLight ? 'hover:text-blue-600' : 'hover:text-blue-400' },
    { icon: Mail, href: 'mailto:m.h.ratul18@gmail.com', label: 'Email', color: isHacker ? 'hover:text-[#00ff41]' : isLight ? 'hover:text-indigo-600' : 'hover:text-cyan-400' }
  ];
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r ${isHacker ? 'from-transparent via-[#00ff41] to-transparent' : 'from-transparent via-cyan-500 to-transparent'} opacity-50`} />
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            <div className="flex items-center gap-2 group">
              <div className={`p-2 rounded-lg border transition-colors ${isHacker ? 'bg-[#000a02] border-[#00ff41]/15 group-hover:border-[#00ff41]' : isLight ? 'bg-slate-50 border-slate-200 group-hover:border-cyan-500' : 'bg-slate-800 border-slate-700 group-hover:border-cyan-500'}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  ...
                  className={`text-left text-sm transition-all hover:translate-x-1 ${isHacker ? 'text-[#00cc32]/60 hover:text-[#00ff41]' : isLight ? 'text-slate-500 hover:text-cyan-600' : 'text-slate-400 hover:text-cyan-400'}`}
                >
                  {link.label}
                </button>
              ))}
        ...
        {/* Bottom Bar */}
        <motion.div variants={itemVariants} className={`mt-12 pt-8 border-t ${isHacker ? 'border-[#00ff41]/15' : isLight ? 'border-slate-200' : 'border-slate-800'}`}>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <ArrowUp className={`w-4 h-4 transition-colors ${isHacker ? 'text-[#00cc32]/60 hover:text-[#00ff41]' : isLight ? 'text-slate-500 hover:text-cyan-400' : 'text-slate-400 hover:text-cyan-400'}`} />
              </motion.button>
            </div>
          </div>
        </motion.div>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute bottom-0 left-1/4 w-64 h-64 rounded-full blur-3xl ${isHacker ? 'bg-[#00ff41]/3' : isLight ? 'bg-cyan-500/3' : 'bg-cyan-500/5'}`} />
        <div className={`absolute bottom-0 right-1/4 w-64 h-64 rounded-full blur-3xl ${isHacker ? 'bg-[#00cc32]/3' : isLight ? 'bg-purple-500/3' : 'bg-purple-500/5'}`} />
      </div>
    </footer>
  );
}