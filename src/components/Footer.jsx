...
export default function Footer() {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r ${isHacker ? 'from-transparent via-[#00ff41] to-transparent' : isCreative ? 'from-transparent via-[#0088ff] to-transparent' : 'from-transparent via-cyan-500 to-transparent'} opacity-50`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            <div className="flex items-center gap-2 group">
              <div className={`p-2 rounded-lg border transition-colors ${isHacker ? 'bg-[#000a02] border-[#00ff41]/15 group-hover:border-[#00ff41]' : isCreative ? 'bg-[#0a0a0a] border-white/10 group-hover:border-[#0088ff]' : isLight ? 'bg-slate-50 border-slate-200 group-hover:border-cyan-500' : 'bg-slate-800 border-slate-700 group-hover:border-cyan-500'}`}>
                <Terminal className={`w-5 h-5 ${isHacker ? 'text-[#00ff41]' : isCreative ? 'text-[#0088ff]' : 'text-cyan-400'}`} />
              </div>
              <div>
                <h3 className={`font-bold ${isHacker ? 'text-[#00ff41]' : isLight ? 'text-slate-900' : 'text-white'}`}>Ratul</h3>
                <p className={`text-xs font-mono ${isHacker ? 'text-[#00cc32]/60' : isCreative ? 'text-[#0088ff] cursive-accent' : 'text-slate-400'}`}>v2.0.0</p>
              </div>
            </div>
          ...
          {/* Center — Quick Links */}
          <motion.div variants={itemVariants} className="space-y-4">
            <h3 className={`text-sm font-semibold uppercase tracking-wider ${isHacker ? 'text-[#00ff41]' : isCreative ? 'text-white' : isLight ? 'text-slate-900' : 'text-white'}`}>
              Quick Links
            </h3>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  ...
                  className={`text-left text-sm transition-all hover:translate-x-1 ${isHacker ? 'text-[#00cc32]/60 hover:text-[#00ff41]' : isCreative ? 'text-slate-400 hover:text-[#0088ff]' : isLight ? 'text-slate-500 hover:text-cyan-600' : 'text-slate-400 hover:text-cyan-400'}`}
                >
                  {link.label}
                </button>
              ))}
              ...
              {socialLinks.map((social) => (
                <motion.a
                  ...
                  className={`group flex items-center gap-2 px-4 py-2 border rounded-lg transition-all ${social.color} ${isHacker
                    ...
                    : isCreative ? 'bg-[#0a0a0a] hover:bg-[#121212] border-white/10 hover:border-[#0088ff]'
        ...
        <motion.div variants={itemVariants} className={`mt-12 pt-8 border-t ${isHacker ? 'border-[#00ff41]/15' : isCreative ? 'border-white/10' : isLight ? 'border-slate-200' : 'border-slate-800'}`}>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <ArrowUp className={`w-4 h-4 transition-colors ${isHacker ? 'text-[#00cc32]/60 hover:text-[#00ff41]' : isCreative ? 'text-slate-400 hover:text-[#0088ff]' : isLight ? 'text-slate-500 hover:text-cyan-400' : 'text-slate-400 hover:text-cyan-400'}`} />
              </motion.button>
            </div>
          </div>
        </motion.div>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute bottom-0 left-1/4 w-64 h-64 rounded-full blur-3xl ${isHacker ? 'bg-[#00ff41]/3' : isCreative ? 'bg-[#0088ff]/3' : isLight ? 'bg-cyan-500/3' : 'bg-cyan-500/5'}`} />
        <div className={`absolute bottom-0 right-1/4 w-64 h-64 rounded-full blur-3xl ${isHacker ? 'bg-[#00cc32]/3' : isCreative ? 'bg-[#ec4899]/3' : isLight ? 'bg-purple-500/3' : 'bg-purple-500/5'}`} />
      </div>
    </footer>
  );
}